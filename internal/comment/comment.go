...
// Service - the struct for our commment service
type Service struct {
	DB *gorm.DB
}
...
// GetCommentBySlug - retrieves all comments matching the slug (path - /article/name/)
...
// PostComment - creates a new comment instance in the database
...
func (s *Service) DeleteComment(ID uint) error {
	...
	if result := s.DB.Model(&comment).Delete(&Comment{}, ID); result.Error != nil {
		return result.Error
	}
